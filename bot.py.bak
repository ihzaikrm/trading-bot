...
async def main():
    ...
    try:
        ...
        if os.path.exists("logs/narrative_state.json"):
            ...
            active_narr = [n[0] if isinstance(n,list) else (n["name"] if isinstance(n,dict) else n) for n in raw[:3]]
    ...
    # --- Save filter status for dashboard ---
    try:
        # ambil data DXY dan momentum terakhir (yang sudah dihitung di loop aset, namun kita ambil ulang agar konsisten)
        dxy_sig, dxy_det = get_dxy_signal()
        mom_sig, mom_det = get_momentum_signal("BTC")  # ambil untuk BTC sebagai representasi
        filter_status = {
            "timestamp": datetime.now().isoformat(),
            "dxy": {
                "signal": dxy_sig,
                "value": dxy_det.get("dxy"),
                "trend": dxy_det.get("trend")
            },
            "momentum": {
                "signal": mom_sig,
                "rsi": mom_det.get("rsi"),
                "ema50_gt_200": mom_det.get("ema50_gt_200")
            },
            "active_narratives": narrative_state.get("active_narratives", []) if 'narrative_state' in locals() else []
        }
        with open("logs/filter_status.json", "w") as f:
            json.dump(filter_status, f, indent=2)
    except Exception as e:
