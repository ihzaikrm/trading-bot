...
Sumber (semua FREE, no API key):
  BTC on-chain:
    - Fear & Greed Index (alternative.me) — sudah ada, diperkaya
...
Cara pakai di signal_engine.py:
    from modules.alternative_data import get_btc_onchain, get_spx_options_flow
...
def _get_binance_long_short() -> dict:
    ...
    try:
        ...
        # Contrarian: terlalu banyak long → crowded → potential reversal
        if ratio >= 1.8:
...
def _get_btc_volume_ratio() -> dict:
    ...
    Volume spike + price drop → kemungkinan outflow ke exchange (bearish).
    Volume spike + price up   → inflow dari exchange (bullish).
    """
...
# ══════════════════════════════════════════════════════════════════════════════
# Wire ke signal_engine.py — tambah ke prompt LLM
# ══════════════════════════════════════════════════════════════════════════════
...
def get_alt_data_for_prompt(asset_name: str) -> str:
    ...
    try:
        if "BTC" in asset_name:
            btc = get_btc_onchain()
            lines.append(f"On-chain: {btc['summary']} → {btc['composite_signal']}")
        elif "GSPC" in asset_name or "SPX" in asset_name:
            spx = get_spx_options_flow()
            lines.append(f"Options flow: {spx['summary']} → {spx['composite_signal']}")
    except Exception as e:
