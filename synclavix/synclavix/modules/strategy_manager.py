...
async def fetch_ai_trading_research() -> str:
    ...
    """
    from modules.llm_clients import call_single_llm
...
async def generate_quarterly_report() -> str:
    ...
    for name in ASSETS:
        ...
        if ev["status"] == "CRITICAL":
            ...
            recommendations.append(
                f"?? *{name}*: WR turun {ev['wr_drop']:.0%} dari baseline — "
    ...
    lines.append("?? _Review temuan di atas secara manual sebelum implementasi_")
